...
try:
    opts, args = getopt.gnu_getopt(sys.argv[1:], 'h',
                                   ('breakdown-by-arg', 'show-plots', 'save-plots', 'plot=', 'title=', 'help',))
...
parse_time_functions = {
    ...
    'ttt': lambda s: datetime.datetime.fromtimestamp(float(s)),
    # (datetime has no format specifier for seconds since epoch (would use something like '%s.%f' if available)
}
...
for filename in args:
    ...
    for line in f.readlines():
        ...
        if not m:
            if not (line.startswith('Process ') or line.endswith('<unfinished ...>') or line.endswith(
                    '= ?') or '--- SIG' in line or '+++ exited with' in line):
